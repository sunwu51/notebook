...
const UNSUPPORTED_OPENAI_RESPONSES_FIELDS = ["background", "context_management", "conversation", "include", "previous_response_id", "prompt", "truncation"] as const;
const UNSUPPORTED_ANTHROPIC_FIELDS = ["cache_control", "container", "inference_geo", "top_k"] as const;
...
export function denormalizeToAnthropicRequest(request: NormalizedRequest): MessageCreateParamsBase {
  ...
  while (index < request.messages.length) {
    const message = request.messages[index];
    if (message.role === "system" || message.role === "developer") {
      for (const part of requireTextOnly(message.parts, "Anthropic system prompt")) systemBlocks.push({ type: "text", text: part.text });
      index += 1;
      continue;
    }
    break;
...
function normalizeAnthropicMessage(message: MessageParam): NormalizedMessage[] {
  ...
  let currentParts: NormalizedMessage["parts"] = [];
  const flush = () => {
  ...
  for (const block of message.content as any[]) {
    ...
    if (block.type === "document") {
      flush();
      if (block.source.type === "url") normalized.push({ role: "user", parts: [{ type: "document_url", url: block.source.url, title: block.title ?? null }] });
      else if (block.source.type === "base64") normalized.push({ role: "user", parts: [{ type: "document_base64", data: block.source.data, mediaType: block.source.media_type, title: block.title ?? null }] });
      else if (block.source.type === "text") currentParts.push(text(block.source.data));
      else if (block.source.type === "content") for (const child of block.source.content) {
...
function denormalizeAnthropicMessage(message: NormalizedMessage): MessageParam[] {
  switch (message.role) {
    ...
    case "tool":
      return [{ role: "user", content: [{ type: "tool_result", tool_use_id: message.toolCallId ?? "", content: denormalizeAnthropicToolResultParts(message.parts) }] as any }];
    case "function":
      return [{ role: "user", content: [{ type: "tool_result", tool_use_id: message.name ?? "function", content: collapseText(requireTextOnly(message.parts, "Anthropic function result")) }] as any }];
    default:
...
function denormalizeAnthropicAssistantParts(message: NormalizedMessage): any[] {
  const blocks: any[] = message.parts.map((part) => ({ type: "text", text: part.type === "text" || part.type === "refusal" ? part.text : collapseText([part]), citations: null }));
...
function denormalizeAnthropicThinking(reasoningEffort: string | null | undefined, maxOutputTokens: number | undefined) {
  ...
  if (!match) fail(`Anthropic thinking only supports budget-based reasoning, got "${reasoningEffort}"`);
