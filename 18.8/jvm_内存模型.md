...
# 2 栈的主要用途
栈、程序计数器和本地方法栈三者是`线程私有`的，每个线程栈的大小可以通过Xss进行设定，上面提到了。栈主要用于存储8大基础类型（boolen,char,byte,short,int,long,float,double这部分又叫局部变量表)以及对象指针，返回地址，另外栈在执行函数的时候会开辟`栈帧`来进行函数内部的上述内容存储，当然也是要存调用的函数的返回值的（即使没有变量来接收这个返回值）。函数中new的对象一般放在堆上,也有栈上的时候后面会说.  下图枣红色是堆，大红色是栈。
